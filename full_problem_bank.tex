...
\item They search for polynomial decision boundaries of degree $\geq$2
\end{enumerate}\vspace{1cm}
...
\subsection*{For fixed measurements X, the slides note that $\sum$$^{i}$ P(C$^{i}$|X) equals:}
\begin{enumerate}
\item P(X)
...
\subsection*{For coin flips with H heads out of N trials, the ML estimate of $\theta$=P(H) is:}
\begin{enumerate}
\item (H-1)/(N-1) always
